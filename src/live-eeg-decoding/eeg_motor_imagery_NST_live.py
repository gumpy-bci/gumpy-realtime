...
class liveEEG():
    def __init__(self, cwd, filename_notlive, flag=0):
        ...
        self.data_notlive_filtbp = gumpy.signal.butter_bandpass(self.data_notlive, lo=7, hi=60)
        ...
        #self.data_notlive_filtno = gumpy.signal.normalize(self.data_notlive_filtno, 'min_max')
    ...
    def fit(self):
        ...
        out_realtime = gumpy.features.sequential_feature_selector(self.features_notlive, self.labels_notlive, 'QuadraticLDA', (6, 30), 5, 'SFFS')
        ...
        self.estimator_object.fit(self.sfs_object.transform(self.features_notlive), self.labels_notlive)
        self.labels_pred_notlive = self.estimator_object.predict(self.sfs_object.transform(self.features_notlive))
    ...
    def classify_live(self, data_live):
        ...
        # only used for 2 classes (left and right), prevent doing classify_live for empty data of 3. class
        #if not data_live.trials:
        #    return 0,0,False
        ...
        # butter-bandpass filtered version of notlive data
        self.data_live_filtbp = gumpy.signal.butter_bandpass(data_live, lo=2, hi=60)

        # apply the notch filter
        self.data_live_filtno = gumpy.signal.notch(self.data_live_filtbp, cutoff=self.notch_f0, Q=self.notch_Q, fs=self.data_notlive.sampling_freq)

        # normalize the data first
        #self.data_live_filtno = gumpy.signal.normalize(self.data_live_filtno, 'min_max')

        self.alpha_bands = np.array(self.alpha_subBP_features(self.data_live_filtno))
        self.beta_bands = np.array(self.beta_subBP_features(self.data_live_filtno))

        # Feature extraction using sub-bands 
        # Method 1: logarithmic sub-band power

        # use 2 seconds of data for live classification (for only 1 second delete self.features3_live und self.features4_live and adapt self.features_live)
        self.w1 = [0,128]
        self.w2 = [128,256]
        self.w3 = [256,384]
        self.w4 = [384,512]
        ...
        self.features1_live = self.log_subBP_feature_extraction(
            self.alpha_bands, self.beta_bands, 
            data_live.trials, data_live.sampling_freq,
            self.w1)
        ...
        self.features3_live = self.log_subBP_feature_extraction(
            self.alpha_bands, self.beta_bands, 
            data_live.trials, data_live.sampling_freq,
            self.w3)

        self.features4_live = self.log_subBP_feature_extraction(
            self.alpha_bands, self.beta_bands, 
            data_live.trials, data_live.sampling_freq,
            self.w4)                                      

        # concatenate the features and normalize the data
        self.features_live = np.concatenate((self.features1_live.T, self.features2_live.T, self.features3_live.T, self.features4_live.T)).T
        self.features_live -= np.mean(self.features_live)
        self.features_live = gumpy.signal.normalize(self.features_live, 'min_max')
        ...
        if False:          
            ...
            self.features1_live= np.array(self.dwt_features(self.data_live_filtno, self.trials_live, 5, self.fs_live, self.w, 3, "db4"))
            self.features2_live= np.array(self.dwt_features(self.data_live_filtno, self.trials_live, 5, self.fs_live, self.w, 4, "db4"))
            # concat the features and normalize
            self.features_live = np.concatenate((self.features1_live.T, self.features2_live.T)).T
            self.features_live -= np.mean(self.features_live)
            self.features_live = gumpy.signal.normalize(self.features_live, 'min_max')

        ### predict label of live trial and check whether it is correct
        if self.pos_fit:
            labels_pred_live = self.estimator_object.predict(self.sfs_object.transform(self.features_live))
            print('Class of Motor Imagery Movement', self.labels_live)
            pred_true = (self.labels_live - labels_pred_live) == 0
